...
二者区别在于对于pod的==重启策略restartPolicy==不同：
...
| Pod名称特征 | 上一级资源类型 | 实例                                 |
| ------- | ------- | ---------------------------------- |
| 随机后缀    | deploy  | `nginx-deploy-5d4f8c9b7-xyz`       |
| 有序数字    | sts     | `redis-master-0`, `redis-master-1` |
| 随机字符串   | ds      | `calico-node-abcde`                |
| 无特定后缀   | 独立po    |  ``kube-controller-manager-thx``   |
...
稳定的网络访问方式：域名：podName.headlessSvcName.nsName.svc.domainName.
...
spec.successfulJobHistoryLimit和spec.failedJobHistoryLimit 历史限制，指定可etcd保留多少完成和失败的pod，默认3和1（批处理类型没有版本一说，不必考虑回滚）

# 题目
1. deployment 属于 k8s 的什么资源？
控制器，pod上一层的资源
...
6. 如何通过 rs 实现 pod 的副本数变成 5？
kubectk scale replicaest <rs-name/> --replicas=5

7. 使用什么命令能够直接修改某个资源的属性，不使用 yaml 文件？
kubectl scale 可以修改副本数
kubectl set 可以修改镜像
如果说任意属性需要kubectl edit <resourse/> <name/>
打补丁用patch
